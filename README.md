...
| `최근뉴스` | 최신 헤드라인 요약 |
| `트렌드점수_최종` | 시장/섹터 중립화까지 반영한 최종 트렌드 점수 |
| `Squeeze_On` | 볼린저 밴드가 켈트너 채널 안에 있어 변동성이 압축된 상태(TRUE) |
| `Squeeze_Off` | 직전 스퀴즈가 방금 해제됐는지 여부(TRUE) |
| `Squeeze_Momentum` | 스퀴즈 모멘텀 히스토그램 값 (양수=상방, 음수=하방) |
| `Squeeze_On(주)` | 주봉 기준 스퀴즈 압축 여부 |
| `Squeeze_Off(주)` | 직전 주봉 스퀴즈 해제 여부 |
| `Squeeze_Momentum(주)` | 주봉 스퀴즈 모멘텀 값 |
| `Squeeze_On(월)` | 월봉 기준 스퀴즈 압축 여부 |
| `Squeeze_Off(월)` | 직전 월봉 스퀴즈 해제 여부 |
| `Squeeze_Momentum(월)` | 월봉 스퀴즈 모멘텀 값 |
| `RSI` | 현재 RSI 값 |
| `RSI(주봉)` | 주봉 RSI 값 |
| `RSI(월봉)` | 월봉 RSI 값 |
...
- **모멘텀**
  ...
  - Squeeze Momentum > 0 : 압축 해제 후 상방 모멘텀, < 0 : 하방 압력
