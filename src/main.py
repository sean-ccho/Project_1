...
def main() -> None:
    ...
    try:
        ...
        _sp500_ranked_snapshot = None  # 두 번째 build_export_dataframe 호출이 덮어쓰기 전에 캡처
        if GOOGLE_SHEETS_SIGNALS_ENABLED:
            ...
            _sp500_ranked_snapshot = getattr(build_export_dataframe, "_last_ranked", None)
        ...
        if GOOGLE_SHEETS_SIGNALS_ENABLED and signals_export is not None:
             ...
             # 고정 종목 및 현재 페이퍼 트레이딩 보유 종목 추가
             try:
                 from paper_trading.portfolio import load_positions
                 pt_positions = load_positions()
                 pt_tickers = [p.get("ticker") for p in pt_positions if p.get("ticker")]
             except Exception:
                 pt_tickers = []
             fixed_tickers = ["NBM.V"] + pt_tickers
        ...
        if GOOGLE_SHEETS_PORTFOLIO_ENABLED:
            if portfolio_tickers and signals_export is not None:
                ...
                portfolio_export = attach_insider_summary(portfolio_export)
        ...
        # ── Paper Trading: SP500 ranked_df 스냅샷 저장 ──
        # 실제 PT 실행은 run_paper_trading.py에서 NASDAQ 풀과 합산 후 통합 처리
        if PAPER_TRADING_ENABLED:
            try:
                ranked_df = _sp500_ranked_snapshot
                if ranked_df is not None and not ranked_df.empty:
                    from paper_trading.runner import save_ranked_snapshot
                    save_ranked_snapshot(ranked_df, source="sp500")
                else:
                    print("[Paper Trading] SP500 ranked 데이터가 없어 스냅샷 저장을 건너뜁니다.")
            except Exception as exc:
                print(f"[Paper Trading] SP500 스냅샷 저장 실패: {exc}")
