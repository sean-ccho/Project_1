...
from screener.config import (
    ...
    PATTERN_SCORE_CAP_BOTTOM,
    PATTERN_SCORE_CAP_MOMENTUM,
...
def attach_signals_and_sort(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calc_bottom_reversal_score(row):
        ...
        # 반전형 패턴 (바닥반등 전략에 적합한 패턴만)
        daily_pat_str = str(row.get("일봉패턴", ""))
        if daily_pat_str and daily_pat_str.lower() != "nan":
            if any(p in _BULLISH_DAILY_REVERSAL for p in daily_pat_str.split(", ")):
                score += W_B["상승패턴"]
        ...
        # 강한 섹터
        if row.get("in_strong_sector", False):
            score += W_B["강한섹터"]
        ...
        # 주봉+월봉 패턴 가산점 (바닥반등용 가중치 + 캡 적용)
        pattern_score = 0.0
        weekly_pat_str = str(row.get("주봉패턴", ""))
        if weekly_pat_str and weekly_pat_str.lower() != "nan":
            for pat in weekly_pat_str.split(", "):
                if pat in WEEKLY_PATTERN_WEIGHTS_BOTTOM:
                    pattern_score += WEEKLY_PATTERN_WEIGHTS_BOTTOM[pat]

        monthly_pat_str = str(row.get("월봉패턴", ""))
        if monthly_pat_str and monthly_pat_str.lower() != "nan":
            for pat in monthly_pat_str.split(", "):
                if pat in MONTHLY_PATTERN_WEIGHTS_BOTTOM:
                    pattern_score += MONTHLY_PATTERN_WEIGHTS_BOTTOM[pat]

        score += min(pattern_score, PATTERN_SCORE_CAP_BOTTOM)
    ...
    def _calc_momentum_score(row):
        ...
        # 돌파/지속형 패턴 (모멘텀 전략에 적합한 패턴만)
        daily_pat_str = str(row.get("일봉패턴", ""))
        if daily_pat_str and daily_pat_str.lower() != "nan":
            if any(p in _BULLISH_DAILY_MOMENTUM for p in daily_pat_str.split(", ")):
                score += W_M["상승패턴"]
        ...
        # 주봉+월봉 패턴 가산점 (모멘텀용 가중치 + 캡 적용)
        pattern_score = 0.0
        weekly_pat_str = str(row.get("주봉패턴", ""))
        if weekly_pat_str and weekly_pat_str.lower() != "nan":
            for pat in weekly_pat_str.split(", "):
                if pat in WEEKLY_PATTERN_WEIGHTS_MOMENTUM:
                    pattern_score += WEEKLY_PATTERN_WEIGHTS_MOMENTUM[pat]

        monthly_pat_str = str(row.get("월봉패턴", ""))
        if monthly_pat_str and monthly_pat_str.lower() != "nan":
            for pat in monthly_pat_str.split(", "):
                if pat in MONTHLY_PATTERN_WEIGHTS_MOMENTUM:
                    pattern_score += MONTHLY_PATTERN_WEIGHTS_MOMENTUM[pat]

        score += min(pattern_score, PATTERN_SCORE_CAP_MOMENTUM)
    ...
    # 전략구분 분류: 전환구간 > 바닥반등 > 모멘텀 > 관망
    threshold = EMAIL_BOTTOM_SCORE_THRESHOLD  # 5.0

    def _classify_strategy(row):
        b = row["바닥반등_적합도"]
        m = row["모멘텀_적합도"]
        if b >= threshold and m >= threshold:
            # 우세 전략으로 재분류 (전환구간 제거 — 늦은 진입 방지)
            return "📉 바닥반등" if b >= m else "📈 모멘텀"
        elif b >= threshold:
            return "📉 바닥반등"
        elif m >= threshold:
            return "📈 모멘텀"
    ...
    out["전략구분"] = out.apply(_classify_strategy, axis=1)

    # 정렬: 전략구분 우선순위 → 구간 내 점수 내림차순
    _strategy_order = {"📈 모멘텀": 0, "📉 바닥반등": 1, "—": 2}
    out["_sort_group"] = out["전략구분"].map(_strategy_order)
    out["_sort_score"] = out[["바닥반등_적합도", "모멘텀_적합도"]].sum(axis=1)
    sorted_out = out.sort_values(
        ["_sort_group", "_sort_score"], ascending=[True, False]
    )

    # Drop internal working columns
    cols_to_drop = [
        judgement.name, recommendation.name,
        "_sort_group", "_sort_score",
    ]
