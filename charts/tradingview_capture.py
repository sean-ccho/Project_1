...
def prepare_tv_state() -> Path:
    """TradingView 로그인 쿠키(세션) 상태 파일을 준비합니다."""
    state_path = Path(__file__).parent.parent / "tv_state.json"
    
    # 환경변수에 세션 ID가 있고 파일이 없다면 동적으로 파일 생성 (GitHub Actions용)
    if not state_path.exists() and "TRADINGVIEW_SESSIONID" in os.environ:
        session_id = os.environ["TRADINGVIEW_SESSIONID"]
        cookie_data = {
            "cookies": [
                {
                    "name": "sessionid",
